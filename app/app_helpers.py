...
def transform_data(
    users_df: pd.DataFrame, experiments_df: pd.DataFrame, compounds_df: pd.DataFrame
) -> pd.DataFrame:
    ...
    derived_results_py["avg_experiment_count"] = [
        avg_experiment_count for _ in user_ids
    ]

    # Mean experiment run time
    total_exp_time_per_user = experiments_df.groupby("user_id")[
        "experiment_run_time"
    ].sum()
    # Add mean experiment time, defaulting to 0 for users with no experiments
    derived_results_py["avg_exp_time_per_user"] = round(
        total_exp_time_per_user.get(user_ids, 0)
        / derived_results_py["total_exp_per_user"],
        2,
    )

    # Prepare compound data
    derived_results_py = prepare_compound_data(
        experiments_df, compounds_df, user_ids, derived_results_py
    )
...
def prepare_compound_data(
    ...
    experiments_df["experiment_compound_ids"] = (
        experiments_df["experiment_compound_ids"]
        .str.split(";")
    ...
    all_compounds_consumed_per_user = experiments_df.groupby("user_id")[
    ...
    derived_results_py["most_consumed_compound_name"] = most_consumed_compound.get(
        user_ids, None
    )
